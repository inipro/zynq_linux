...
class PmodCLS:
	...
	def __init__(self, devname):
		self.serial = Serial(devname, baudrate=9600)
	...
	def writeStringAtPos(self, idxRow, idxCol, strLn):
		...
		if bResult == PmodCLS.LCDS_ERR_SUCCESS:
			...
			self.serial.write(bytearray(stringToSend[:7+length]))
		return bResult
	...
	def cursorModeSet(self, setCursor, setBlink):
		...
		if not setCursor:
			...
			self.serial.write(bytearray([PmodCLS.ESC, PmodCLS.BRACKET, ord('0'), PmodCLS.CURSOR_MODE_CMD]))
		elif setCursor and not setBlink:
			# send the command for display on and blink off
			self.serial.write(bytearray([PmodCLS.ESC, PmodCLS.BRACKET, ord('1'), PmodCLS.CURSOR_MODE_CMD]))
		else:
			# send the command for display and blink on
			self.serial.write(bytearray([PmodCLS.ESC, PmodCLS.BRACKET, ord('2'), PmodCLS.CURSOR_MODE_CMD]))

	def displayClear(self):
		self.serial.write(bytearray([PmodCLS.ESC, PmodCLS.BRACKET, ord('0'), PmodCLS.DISP_CLR_CMD]))

	def displayMode(self, charNumber):

		if charNumber:
			# wrap line at 16 characters
			self.serial.write(bytearray([PmodCLS.ESC, PmodCLS.BRACKET, ord('0'), PmodCLS.DISP_MODE_CMD]))
		else:
			# wrap line at 40 characters
			self.serial.write(bytearray([PmodCLS.ESC, PmodCLS.BRACKET, ord('1'), PmodCLS.DISP_MODE_CMD]))
	...
	def defineUserChar(self, strUserDef, charPos):
		...
		if charPos >= 0 and charPos <= 7:
			...
			self.serial.write(bytearray(rgcCmd))
			bResult = PmodCLS.LCDS_ERR_SUCCESS
		else:
