...
apt-get update
apt-get -y upgrade
DEBIAN_FRONTEND=noninteractive apt-get -y install lsb-release vim nano sudo openssh-server udev usbutils u-boot-tools device-tree-compiler kmod net-tools wpasupplicant parted rfkill lshw wireless-tools gcc g++ cmake python git i2c-tools iputils-ping lxterminal leafpad linux-firmware
